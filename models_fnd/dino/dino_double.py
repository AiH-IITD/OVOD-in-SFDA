...
class DINODouble(nn.Module):
    ...
    def forward(self, images, inputmasks, targets:List=None, dru_teacher: bool = False):
        ...
        outputs_coord_expert_list = []
        for dec_lid, (layer_ref_sig, layer_bbox_embed, layer_hs) in enumerate(zip(reference[:-1], self.bbox_embed_expert, hs)):
            layer_delta_unsig = layer_bbox_embed(layer_hs)
            layer_outputs_unsig = layer_delta_unsig  + inverse_sigmoid(layer_ref_sig)
            layer_outputs_unsig = layer_outputs_unsig.sigmoid()
            outputs_coord_expert_list.append(layer_outputs_unsig)
        outputs_coord_expert_list = torch.stack(outputs_coord_expert_list)        


        # outputs_class = self.class_embed(hs)
        outputs_class = torch.stack([layer_cls_embed(layer_hs) for
                                     layer_cls_embed, layer_hs in zip(self.class_embed, hs)])
        outputs_class_expert = torch.stack([layer_cls_embed(layer_hs) for
                                     layer_cls_embed, layer_hs in zip(self.class_embed_expert, hs)])
        if self.dn_number > 0 and dn_meta is not None:
            outputs_class, outputs_coord_list = \
                dn_post_process(outputs_class, outputs_coord_list,
                                dn_meta,self.aux_loss,self._set_aux_loss)
            outputs_class_expert, outputs_coord_expert_list = \
                dn_post_process(outputs_class_expert, outputs_coord_expert_list,
                                dn_meta,self.aux_loss,self._set_aux_loss)
        out = {'logit_all': outputs_class[-1], 'boxes_all': outputs_coord_list[-1]}
        out_expert = {'logit_all': outputs_class_expert[-1], 'boxes_all': outputs_coord_expert_list[-1]}
        if self.aux_loss:
            out['aux_outputs'] = self._set_aux_loss(outputs_class, outputs_coord_list)
            out_expert['aux_outputs'] = self._set_aux_loss(outputs_class_expert, outputs_coord_expert_list)
        ...
        dont_copy = ["logit_all", "boxes_all", "aux_outputs"]
        for k, v in out.items():
            if k not in dont_copy:
                out_expert[k] = v
        return out, out_expert
